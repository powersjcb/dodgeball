...
MovingObject.prototype.isCollided = function (object) {
  ...
  if (dist < (this.radius + object.radius)) {
    if ( this.collisions.length > 0 && this.collisions.length % 5 === 0) {
      var cols = this.collisions.reduce(function (previous, current) {return current;});
      console.log( cols + "% interference");
      this.collisions = [];
    }

    this.collisions.push((this.radius + object.radius - dist) / (object.radius + object.radius) * 100);
    return true;
  } else {
    return false;
  }
